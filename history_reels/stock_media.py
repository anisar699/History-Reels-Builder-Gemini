...
def download_clip_for_query(query, index):
    ...
    providers = []
    allowed = getattr(config, "ALLOWED_SOURCES", ["pexels", "pixabay", "google", "pinterest", "wikimedia_image"])
    
    if config.MEDIA_PREFERENCE in ["mixed", "videos"]:
        if "storyblocks" in allowed and getattr(config, "STORYBLOCKS_PUBLIC_KEY", None) and getattr(config, "STORYBLOCKS_PRIVATE_KEY", None):
            providers.append("storyblocks")
        if "pexels" in allowed and getattr(config, "PEXELS_API_KEY", None):
            providers.append("pexels")
        if "pixabay" in allowed and getattr(config, "PIXABAY_API_KEY", None):
        ...
        if provider == "storyblocks":
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    results = [r_item for r_item in results if str(r_item.get('id')) not in config.DOWNLOADED_VIDEO_IDS]
                    if results:
                        item = results[0]
                        preview_url = item.get("preview_url")
                        if preview_url and download_file_with_retry(preview_url, save_path):
                            config.DOWNLOADED_VIDEO_IDS.add(str(item.get('id')))
                            config.VIDEO_ATTRIBUTIONS.append(f"Storyblocks Video: {item.get('title')} (ID: {item.get('id')})")
                            return True
            except Exception as e:
                ...
                if r.status_code == 200:
                    ...
                    if candidates:
                        ...
                        if link and download_file_with_retry(link, save_path):
                            ...
                            config.VIDEO_ATTRIBUTIONS.append(f"Pexels Video by {user_info.get('name', 'Unknown')} ({selected_v.get('url', '')})")
                            return True
            except Exception as e:
                ...
                if r.status_code == 200:
                    ...
                    if candidates:
                        ...
                        if link and download_file_with_retry(link, save_path):
                            ...
                            config.VIDEO_ATTRIBUTIONS.append(f"Pixabay Video by {selected_h.get('user', 'Unknown')} (ID: {selected_h.get('id')})")
                            return True
            except Exception as e:
            ...
            try:
                ...
                for selected_url in urls[:10]:
                    if selected_url in config.DOWNLOADED_VIDEO_IDS:
                        continue
                    ...
                    if download_file_with_retry(selected_url, img_save_path):
                        config.DOWNLOADED_VIDEO_IDS.add(selected_url)
                        config.VIDEO_ATTRIBUTIONS.append(f"Google/Bing Image: {selected_url[:80]}...")
                        return True
            except Exception as e:
            ...
            try:
                ...
                for selected_url in urls[:10]:
                    ...
                    if download_file_with_retry(selected_url, img_save_path):
                        config.DOWNLOADED_VIDEO_IDS.add(selected_url)
                        config.VIDEO_ATTRIBUTIONS.append(f"Pinterest Image: {selected_url[:80]}...")
                        return True
            except Exception as e:
                ...
                for page_id, page_data in pages.items():
                    ...
                    if info:
                        ...
                        if img_url and img_url.lower().endswith(('.jpg', '.jpeg', '.png')):
                            if img_url in config.DOWNLOADED_VIDEO_IDS:
                                continue
                            ext = ".png" if ".png" in img_url.lower() else ".jpg"
                            img_save_path = os.path.join(config.TOPIC_TEMP_DIR, f"raw_clip{index}{ext}")
                            if download_file_with_retry(img_url, img_save_path):
                                config.DOWNLOADED_VIDEO_IDS.add(img_url)
                                config.VIDEO_ATTRIBUTIONS.append(f"Wikimedia Commons Image: {page_data.get('title', '')}")
                                return True
            except Exception as e:
            ...
            unsplash_key = getattr(config, "UNSPLASH_API_KEY", None)
            if unsplash_key:
                    ...
                    for item in results:
                        ...
                        if img_url:
                            ...
                            if download_file_with_retry(img_url, img_save_path):
                                ...
                                config.VIDEO_ATTRIBUTIONS.append(f"Unsplash Image by {author}")
                                return True
                except Exception as e:
                ...
                for item in items[:10]:
                    ...
                    if links:
                        ...
                        if img_url:
                            ...
                            if download_file_with_retry(img_url, img_save_path):
                                config.DOWNLOADED_VIDEO_IDS.add(img_url)
                                config.VIDEO_ATTRIBUTIONS.append(f"NASA Image")
                                return True
            except Exception as e:
                ...
                for doc in docs:
                    ...
                    if identifier:
                        ...
                        if mp4_file:
                            ...
                            if video_url in config.DOWNLOADED_VIDEO_IDS:
                                continue
                            save_path = os.path.join(config.TOPIC_TEMP_DIR, f"raw_clip{index}.mp4")
                            if download_file_with_retry(video_url, save_path):
                                config.DOWNLOADED_VIDEO_IDS.add(video_url)
                                config.VIDEO_ATTRIBUTIONS.append(f"Internet Archive Video: {doc.get('title', 'Unknown')}")
                                return True
            except Exception as e:
    ...
    if query != "cinematic":
        ...
        return download_clip_for_query("cinematic", index)
