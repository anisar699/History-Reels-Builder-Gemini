...
Phase 14 is complete: brand logo path contract (`WATERMARK_PATH` + stable `watermark.png`), content-hash bumper/logo uploads, bumper stitch normalized to AAC 48 kHz with re-encode concat (no stream-copy), sensitive settings strip `PRIVATE_KEY`/`PUBLIC_KEY`, stock-media log redaction, session key override enforcement in the dashboard, metrics key alignment, cancel helper cleanup, and HTML-escaped gallery titles.

Phase 15 is complete: shared `ffmpeg_runner` (timeouts, stderr tails, structured `RenderError`, job-process kill registry), per-subprocess FONTCONFIG only (no global env mutation), atomic SQLite job claim, terminal status FSM (late success cannot overwrite cancel), hard cancel for process workers + tracked media children, reconcile of stuck `running` jobs, image dimension quality gates with real high/balanced floors, sanitized Gemini/Groq/Ollama/OpenRouter errors, validation-fail cleanup, and `--keep-workspace` / `KEEP_WORKSPACE` retention.
